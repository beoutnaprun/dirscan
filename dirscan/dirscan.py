...
parser.add_argument('-t', '--thread', help='\t指定线程 默认2', default='2')
...
def request_url(url, proxy):
    try:
        ...
        if proxy == "d":
            req = requests.get(url, verify=False, allow_redirects=False, headers=header, proxies=proxys, timeout=10)
            ...
            req = requests.get(url, verify=False, allow_redirects=False, headers=header, timeout=10)
            if req.status_code == 404:
                # print(f"\033[1;36;40m{req.status_code}        {url}\033[0m")
                pass
            elif req.status_code == 400:
    ...
    if a == "y" or a == "Y":
        if args.url != "":
            ...
            if args.proxy:
                ...
                threq(req_list(urls), int(args.thread), proxy)
            else:
                print(
                ...
                print("=" * 30 + "正在爆破目录~~请稍等~~" + "=" * 30)
                threq(req_list(urls), int(args.thread), {})
        else:
...
def runproxy():
    ...
    if a == "y" or a == "Y":
        if args.url != "":
            ...
            print(
                f"\033[1;36;40m爆破地址: \033[0m \033[1;34;40m{args.url}\033[0m      \033[1;36;40m爆破线程: \033[0m \033[1;34;40m{args.thread}\033[0m  \033[1;36;40m爆破代理: \033[0m \033[1;34;40m代理池\033[0m ")
            print("=" * 30 + "正在爆破目录~~请稍等~~" + "=" * 30)
            threq(req_list(urls), int(args.thread), "d")
        else:
            print("Error : 请使用-h查看帮助信息")
